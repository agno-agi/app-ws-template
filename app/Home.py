...
from agno.tools.streamlit.components import (
    check_password,
...
st.title("AI Agent")
st.markdown("##### :orange_heart: built using [Agno](https://github.com/phidatahq/phidata)")
...
def main() -> None:
    ...
    st.session_state["example_agent_session_id"] = example_agent.session_id
    if example_agent.session_id is not None:
        try:
            st.session_state["example_agent_session_id"] = example_agent.load_session()
        except Exception as e:
            logger.error(f"Could not create Agent session, is the database running? {e}")
            st.warning(f"Could not create Agent session, is the database running? {e}")
            return
    ...
    if example_agent.storage:
        ...
        if (
            ...
            and st.session_state["example_agent_session_id"] is not None
        ):
